...
title('A7', 'simple sunshine with a json error')
exitcode, output = execute(fixed + '--root test/A3_test_simple_bad_json --package a --locate', ErrorCode.SYNTAX_ERROR)
...
exitcode, output = execute(fixed + '--root exception/B2_test_json_error --package a --check', ErrorCode.SYNTAX_ERROR)

title('B3', 'missing name')
exitcode, output = execute(fixed + '--root exception/B3_test_missing_name --package a --check', ErrorCode.MISSING_INPUT)

title('B4', 'missing version')
exitcode, output = execute(fixed + '--root exception/B4_test_missing_version --package a --check', ErrorCode.MISSING_INPUT)
...
exitcode, output = execute(fixed + '--root test/C3_test_different_buildtypes --package a:development:anyarch:debug:0.1.2 --tree', ErrorCode.OK)
...
exitcode, output = execute(fixed + '--root test/F2_test_duplicate_package_slotted_ok --package a:anytrack:x86 --tree', ErrorCode.OK)
