...
err, output = execute(fixed + '--root test/test_obsoleta:. --package testsuite --check', ErrorCode.OK)

title(10, 'simple sunshine --tree')
err, output = execute(fixed + '--root test/test_simple --package \'*\' --tree', ErrorCode.OK)
test(len(output[:-1].split('\n')), 6)

title(11, 'simple sunshine --list')
err, output = execute(fixed + '--root test/test_simple --package a --check', ErrorCode.OK)

title(12, 'simple sunshine --buildorder')
err, output = execute(fixed + '--root test/test_simple --package a --buildorder', ErrorCode.OK)
...
err, output = execute(fixed + '--root test/test_simple --package a --buildorder --printpaths', ErrorCode.OK)
output = output.split('\n')
...
err, output = execute(fixed + '--root exception/test_json_error --package a --check', ErrorCode.SYNTAX_ERROR)

title(22, 'missing name')
err, output = execute(fixed + '--root exception/test_missing_name --package a --check', ErrorCode.MISSING_INPUT)

title(23, 'missing version')
err, output = execute(fixed + '--root exception/test_missing_version --package a --check', ErrorCode.MISSING_INPUT)

title(24, 'missing package')
err, output = execute(fixed + '--root test/test_missing_package --package a --check', ErrorCode.PACKAGE_NOT_FOUND)
...
err, output = execute(fixed + '--root test/test_different_arch_dev_arch1_arch2 --package a --check', ErrorCode.PACKAGE_NOT_FOUND)

title(32, "different buildtypes are ok for non-production build")
err, output = execute(fixed + '--root test/test_different_buildtypes --package a --tree', ErrorCode.OK)
test("b:testing:anyarch:release:0.1.2" in output)

title(33, "different buildtypes are not ok for production build")
err, output = execute(fixed + '--root test/test_different_buildtypes_production --package a --tree', ErrorCode.PACKAGE_NOT_FOUND)

title(35, 'failing since a <<< c-1.2.4 but a <<< b <<< c-1.2.3 and b <<< d <<< c-1.2.4')
err, output = execute(fixed + '--root test/test_multiple_versions --package a --check', ErrorCode.MULTIPLE_VERSIONS)
print(output)
...
err, output = execute(fixed + '--root test/test_multiple_versions --package d --check', ErrorCode.OK)

title(37, 'fail to list buildorder as there are a circular dependency')
err, output = execute(fixed + '--root test/test_circular_dependency --package \* --buildorder', ErrorCode.CIRCULAR_DEPENDENCY)

title(38, 'named architecture -> named architecture -> "anyarch" is ok')
err, output = execute(fixed + '--root test/test_arch_noarch --package a:anytrack:beos --check', ErrorCode.OK)

title(40, 'find b in version 0.2.0')
err, output = execute(fixed + '--root test/range_find_newest --package a --tree', ErrorCode.OK)
test("b:anytrack:anyarch:unknown:0.2.0" in output)

title(41, 'find b in version 0.2.0')
err, output = execute(fixed + '--root test/range_find_newest --package c --tree', ErrorCode.OK)
test("b:anytrack:anyarch:unknown:0.2.0" in output)

title(42, 'find b in version 0.2.0')
err, output = execute(fixed + '--root test/range_find_newest --package d --tree', ErrorCode.OK)
test("b:anytrack:anyarch:unknown:0.2.0" in output)

title(43, 'find b in version 0.2.0')
err, output = execute(fixed + '--root test/range_find_newest --package e --tree', ErrorCode.OK)
test("b:anytrack:anyarch:unknown:0.2.0" in output)

title(50, 'find b in version 0.1.2, higher versions exist but they are discontiued and defective respectively')
err, output = execute(fixed + '--root test/test_discontinued_defective --package a --tree', ErrorCode.OK)
...
err, output = execute(fixed + '--root test/test_different_buildtypes --package a:development:anyarch:debug:0.1.2 --tree', ErrorCode.OK)

title(101, "testing compact 4:5 (using different buildtypes are ok for non-production build)")
err, output = execute(fixed + '--root test/test_different_buildtypes --package a:development:anyarch:debug --tree', ErrorCode.OK)

title(102, "testing compact 3:5 (using different buildtypes are ok for non-production build)")
err, output = execute(fixed + '--root test/test_different_buildtypes --package a:development:anyarch --tree', ErrorCode.OK)

title(103, "testing compact 2:5 (using different buildtypes are ok for non-production build)")
err, output = execute(fixed + '--root test/test_different_buildtypes --package a:development --tree', ErrorCode.OK)

title(104, "testing compact with package path (using different buildtypes are ok for non-production build)")
err, output = execute(fixed + '--root test/test_different_buildtypes --path test/test_different_buildtypes/a --tree', ErrorCode.OK)

title(105, "testing compact with package path (using different buildtypes are ok for non-production build)")
err, output = execute(fixed + '--root test/test_different_buildtypes --path test/test_different_buildtypes/a --tree', ErrorCode.OK)

title(200, "duplicate package")
err, output = execute(fixed + '--root test/test_duplicate_package --package a --tree', ErrorCode.DUPLICATE_PACKAGE)

title(300, "duplicate package - slot test 1, fails since 'a' is not unique enough")
err, output = execute(fixed + '--root test/test_duplicate_package_slotted_ok --package a --tree', ErrorCode.DUPLICATE_PACKAGE)
...
err, output = execute(fixed + '--root test/test_duplicate_package_slotted_ok --package a:anytrack:x86 --tree', ErrorCode.OK)

title(303, "duplicate package - slot test 1, fail, b in x86_64 not found")
err, output = execute(fixed + '--root test/test_duplicate_package_slotted_ok --package a:anytrack:x86_64 --tree', ErrorCode.PACKAGE_NOT_FOUND)

title(400, "duplicate package - slot test 2, fail, a in x86 not found")
err, output = execute(fixed + '--root test/test_duplicate_package_slotted_missing_b --package a:anytrack:x86 --tree', ErrorCode.PACKAGE_NOT_FOUND)

title(401, "duplicate package - slot test 2, fail, b in x86_64 not found")
err, output = execute(fixed + '--root test/test_duplicate_package_slotted_missing_b --package a:anytrack:x86_64 --tree', ErrorCode.PACKAGE_NOT_FOUND)
