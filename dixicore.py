...
class Dixi:
    ...
    def to_c_header(self):
        ...
        include.append('#define OBS_PACKAGE_NAME "%s"' % self.dict['name'])
        include.append('#define OBS_PACKAGE_VERSION "%s"' % self.dict['version'])
        ...
        try:
            ...
            for i in range(self.package.get_nof_dependencies()):
                dep_names.append('"%s"' % self.dict['depends'][i]['name'])
                dep_versions.append('"%s"' % self.dict['depends'][i]['version'])
