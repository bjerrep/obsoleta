#!/usr/bin/env python3
import os
from obsoleta.test.test_common import TESTDATA_PATH, title, test_true, test_ok, test_eq, test_error, populate_local_temp
from obsoleta.common import Position, Conf
...
populate_local_temp('C5_test_multiple_versions')
obsoleta = ObsoletaApi(conf, args)
...
def test_bump(compact, path, bump=False, skip_ranged_versions=False, package_for_check=None, dryrun=False):
    ...
    if package_for_check:
        error, _messages = obsoleta.check(package_for_check)
    else:
        error, _messages = obsoleta.check(compact)
    test_ok(error)
    return result
...
message = test_bump('b', 'G2_test_slot')
test_eq(message,
...
message = test_bump('d:::linux', 'G2_test_slot')
test_eq(message,
...
message = test_bump('f', 'G2_test_slot', bump=True, package_for_check='a')
test_eq(message,
...
message = test_bump('f', 'G2_test_slot', package_for_check='a')
test_eq(message,
    ['bumping package "f" (f:6.6.6:anytrack:linux:unknown) from 6.6.6 to 6.6.7 in "f"',
     'bumping dependency f:6.6.6:anytrack:linux:unknown in downstream "e" from 6.6.6 to 6.6.7 in "e"'])

title('TOA 6C', 'bump multislot')
message = test_bump('b:::windows', 'G1_test_multislot', package_for_check='a')
test_eq(message,
...
message = test_bump('b', 'G1_test_multislot', bump=True, package_for_check='a')
test_eq(message,
...
message = test_bump('b', 'G1_test_multislot')
test_eq(message,
    ['bumping package "b" (b:1.1.1:anytrack:linux:unknown) from 1.1.1 to 1.1.2 in "b_multi_out_of_source"',
     'bumping package "b" (b:1.1.1:anytrack:windows:unknown) from 1.1.1 to 1.1.2 in "b_multi_out_of_source"',
     'bumping dependency b:1.1.1:anytrack:windows:unknown in downstream "a" from 1.1.1 to 1.1.2 in "a"'])
...
message = test_bump('y:::windows', 'G1_test_multislot', bump=True)
test_eq(message,
...
message = test_bump('z', 'G1_test_multislot')
test_eq(message,
...
message = test_bump('w', 'G1_test_multislot', skip_ranged_versions=True)
test_eq(message,
...
populate_local_temp('B5_test_missing_package')
obsoleta = ObsoletaApi(conf, args)
...
populate_local_temp('A4_dont_get_confused')
obsoleta = ObsoletaApi(conf, args)
error, messages = obsoleta.tree('a')
test_eq(messages,
...
keep_track_conf = Conf(f'{TESTDATA_PATH}/test_keep_track.conf')
...
populate_local_temp('C6_test_multiple_versions_with_allow_duplicates')
obsoleta = ObsoletaApi(conf, args)
errors, messages = obsoleta.buildorder('a')
test_ok(errors[0])
test_eq(str(messages),'[c:1.2.3:production:anyarch:unknown, c:1.2.4:production:anyarch:unknown, \
